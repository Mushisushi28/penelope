  ...
  steps: [
    {
      id: "greet",
      role: "agent",
      template: "Hey {customer_first_name}! Thanks for reaching out to {business_name}. What kind of vehicle do you have?",
    },
    {
      id: "capture_vehicle",
      role: "customer",
      extract: [{ field: "vehicle_year" }, { field: "vehicle_make" }, { field: "vehicle_model" }],
    },
    {
      id: "capture_service",
      role: "agent",
      template: "Nice! What service are you looking for? Exterior wash, interior deep clean, or full detail?",
    },
    {
      id: "extract_service",
      role: "customer",
    ...
    },
    {
      id: "quote",
      role: "agent",
      template: "Here's a quick estimate for a {service_requested} on your {vehicle_year} {vehicle_make} {vehicle_model}. Want to lock in a time?",
    },
  ],
};
...
function makePerfectRecording(): Recording {
  ...
  return {
    ...
    messages: [
      ...
      { role: "agent", content: "Here's a quick estimate for a full detail on your 2020 Honda Civic. Want to lock in a time?" },
    ],
...
describe("recorder integration", () => {
  it("createRecorder captures turns correctly", async () => {
    ...
    const agentResponses = ["Hey! What vehicle do you have?", "Great, full detail — want a time slot?"];
    let callCount = 0;
    const mockAgent = async (_input: string) => agentResponses[callCount++] ?? "...";
    ...
    expect(recording.messages).toHaveLength(4);
