...
- **Registry**: merges local seed items with a read-only remote index hosted at `penelope-marketplace`.
- **Installer**: downloads, verifies SHA-256, writes to `tenants/<id>/sandbox/`. Requires TOTP before promoting to live.
...
// Browse all items (local seed + remote)
const items = await loadRegistry();

// Install a procedure into sandbox
...
// Promote to live (requires 6-digit TOTP)
...
penelope marketplace promote mobile-detail-v1   # requires PENELOPE_TOTP env var
...
1. Fork `penelope-marketplace`.
2. Add your manifest JSON to `items/` and your payload YAML to `payloads/`.
3. Generate a SHA-256 of your payload and set it in the manifest.
4. Open a PR. Maintainers review for safety and vertical fit.
5. On merge, the `index.json` regenerates and the item is live.

Items must be MIT or Apache-2.0 licensed and must not contain hardcoded credentials.
