 ...
 * penelope marketplace list | install <id> | promote <id> | audit
 */
...
async function cmdList(): Promise<void> {
  ...
  if (items.length === 0) {
    console.log("No items in registry.");
    return;
  }
...
async function cmdInstall(id: string): Promise<void> {
  ...
  if (!manifest) { console.error(`Item "${id}" not found.`); process.exit(1); }
  const result = await installFromUrl(manifest, { tenantId: TENANT_ID, tenantsRoot: TENANTS_ROOT, sandbox: true });
...
async function cmdPromote(id: string): Promise<void> {
  ...
  if (!totpCode) { console.error("PENELOPE_TOTP env var required."); process.exit(1); }
  const result = await promote({ tenantId: TENANT_ID, tenantsRoot: TENANTS_ROOT, manifest, totpCode });
  console.log(result.message);
}
